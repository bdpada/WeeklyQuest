...
import { changeCurrentUserEmail, changeCurrentUserPassword, getCurrentUserProfile, updateCurrentUserProfile } from '../services/user.service.js';
...
export async function updateMyEmail(req: Request<object, object, { email: string; currentPassword: string }>, res: Response) {
  const user = await changeCurrentUserEmail(req.body, currentUser(req));
  res.status(200).json({ user });
}
